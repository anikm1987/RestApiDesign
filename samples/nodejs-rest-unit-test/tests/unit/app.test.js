...
test('/fizzbuzz get - results based on number - test Buzz',async done=>{
    const { body } = await request.get("/fizzbuzz?number=5");
    expect(body).toEqual({ message: "Buzz" })
    done()
})
test('/fizzbuzz get - results based on number - test Fizz',async done=>{
    const { body } = await request.get("/fizzbuzz?number=aSFG");
    expect(body).toEqual({ message: "Fizz" })
    done()
})

test('/fizzbuzz get - results based on number - test FizzBuzz',async done=>{
    const { body } = await request.get("/fizzbuzz?number=15");
    expect(body).toEqual({ message: "FizzBuzz" })
    done()
})

test('/fizzbuzz get - results based on number - test normal number',async done=>{
    const { body } = await request.get("/fizzbuzz?number=2");
    expect(body).toEqual({ message: "2" })
    done()
})